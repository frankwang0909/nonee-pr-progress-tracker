...
const SOURCES = [
	{
		key: 'nov-2025',
		sheetId: '1Zq7eovhjfSTZkRm6WwMkRLYSGW5LgiSDEf6WBHudvcA',
		gid: '342524790',
	},
	{
		key: 'oct-2025',
		sheetId: '10CBcIhFt3q5iQwZwvDcGntF1l2pnNfwW0ehqvPp_oMU',
		gid: '0',
	},
	{
		key: 'sep',
		sheetId: '1doyD3zSDi6D-dUsx0lkIzbYHmkFL06ay24-mKvMDjTA',
		gid: '327230977',
	},
	{
		key: 'aug',
		sheetId: '1e6NVJS8ZZQ1zB5n--C2AjsZS7jFXnp3lesM0KHQBuNM',
		gid: '327230977',
	},
	{
		key: 'jul',
		sheetId: '1Sxq-4-k34GdfRcsvJa2Ply4u9pqEpgFkmiewOur_BW8',
		gid: '1137435725',
	},
	{
		key: 'jun',
		sheetId: '18n_DsN0ui1YGqlygIUmF9VIV43ZhpsSWdaEVSTxB-vk',
		gid: '0',
	},
	{
		key: 'may',
		sheetId: '1ax_Xvp01lg3sFRkZioz5w_c5oGlVLwbNkhgmxMw7GN4',
		gid: '1137435725',
	},
	{
		key: 'apr',
		sheetId: '1m_CSwVXZLAyzw1jiDA6k0WgZH_fk2DBDy39TPcLWE30',
		gid: '0',
	},
	{
		key: 'mar',
		sheetId: '1P0sJOc11nCG3bl8jn0wY4XmhAKhmj-i5B7Yr6_tA9v8',
		gid: '0',
	},
	{
		key: 'feb',
		sheetId: '19VoIhvQo5X7SRT9MNW9DPAP7WKgrLE5cgC7giYLctCM',
		gid: '0',
	},
	{
		key: 'jan',
...
const MAX_FD_TO_P1_DAYS = 500;
const MAX_P1_TO_P2_DAYS = 500;
const MAX_P2_TO_ECOPR_DAYS = 500;
const MIN_FRONTIER_SAMPLE_COUNT = 1;

const SHIFTED_FD_SOURCE_KEYS = new Set([
	'may',
	'jun',
	'jul',
	'aug',
...
// dec-new, oct, nov share the same column layout:
// col_11=PAL, col_12=FD, col_13=P1, col_14=P2, col_16=eCOPR
const DEC_LAYOUT_SOURCE_KEYS = new Set(['dec-new', 'oct', 'nov']);
...
const normalizeReturnReason = (adr) => {
	...
	if (uniqueReasons.length > 0) {
		if (hasAdr)
			return `${expandAbbreviationToken('ADR')} - ${uniqueReasons.join('; ')}`;
	...
	const orderedAbbr = [...ABBR_EXPLANATIONS.keys()].sort(
		(a, b) => b.length - a.length,
	);
	orderedAbbr.forEach((abbr) => {
		const regex = new RegExp(`\\b${abbr}\\b`, 'gi');
		enriched = enriched.replace(regex, (match) =>
			expandAbbreviationToken(match.toUpperCase()),
		);
	});
	return enriched;
};
...
const extractFdDate = (record) => {
	const sourceKey = getRecordSourceKey(record);
	if (sourceKey && DEC_LAYOUT_SOURCE_KEYS.has(sourceKey)) {
		// dec/oct/nov layout: col_11=PAL, col_12=FD, col_13=P1, col_14=P2, col_16=eCOPR
...
const extractP1Date = (record) => {
	...
	if (sourceKey && DEC_LAYOUT_SOURCE_KEYS.has(sourceKey)) {
		// dec/oct/nov layout: col_11=PAL, col_12=FD, col_13=P1, col_14=P2, col_16=eCOPR
		return extractDateFromKeys(record, ['p1', 'portal1', 'portal_1', 'col_13']);
	}
	if (sourceKey === 'jan') {
...
const extractEcoprDate = (record) => {
	...
	if (sourceKey && DEC_LAYOUT_SOURCE_KEYS.has(sourceKey)) {
		// dec/oct/nov layout: col_11=PAL, col_12=FD, col_13=P1, col_14=P2, col_16=eCOPR
		return extractDateFromKeys(record, ['col_16', 'ecopr', 'e_copr', 'copr']);
	}
	if (sourceKey === 'jan') {
		// jan layout: FD=col_15, P1=col_16, P2=col_17, PPR=col_18, eCOPR=col_19
		return extractDateFromKeys(record, ['ecopr', 'e_copr', 'copr', 'col_19']);
	}
	if (sourceKey && FEB_MAR_FD_SOURCE_KEYS.has(sourceKey)) {
		// Feb/Mar layout: FD, P1, P2, PPR, eCOPR, PR Card
...
const extractPrCardDate = (record) => {
	const sourceKey = getRecordSourceKey(record);
	if (sourceKey === 'jan') {
		// jan layout: eCOPR=col_19, PR Card=col_20
		return extractDateFromKeys(record, [
			'pr_card',
			'prcard',
			'pr_card_date',
...
const isValidPrCardStage = (record, submission, prCardDate) => {
	if (!isStageLagWithin(submission, prCardDate, MAX_PRCARD_LAG_DAYS))
		return false;
...
const shouldApplyAorLagFilter = (record) =>
	getRecordSourceKey(record) !== 'apr';
...
const getFrontierRecordIdentity = (record, submission) =>
	[
		...
		String(record.province ?? '')
			.trim()
			.toLowerCase(),
		String(record.pnp_category ?? record.stream ?? '')
			.trim()
			.toLowerCase(),
...
const buildStageFrontierByDay = (records, getStageDate, isValidStage) => {
	...
	for (const record of records) {
		const submission =
			toValidDate(record.__submissionDate) ?? toValidDate(record.__firstDate);
		...
		if (!existing) {
			map.set(submissionDay, {
				identities: new Set([identity]),
				latestStageDay: stageDay,
			});
			continue;
		}
...
const extractRemarksStats = (records) => {
	...
	for (const record of records) {
		...
		const fdToP1 = fdDate && p1Date ? diffDaysFloor(toLocalDayString(fdDate), toLocalDayString(p1Date)) : null;
		if (fdToP1 !== null && fdToP1 >= 1 && fdToP1 <= MAX_FD_TO_P1_DAYS)
			fdToP1Days.push(fdToP1);

		const p1ToP2 = p1Date && p2Date ? diffDaysFloor(toLocalDayString(p1Date), toLocalDayString(p2Date)) : null;
		if (p1ToP2 !== null && p1ToP2 >= 1 && p1ToP2 <= MAX_P1_TO_P2_DAYS)
			p1ToP2Days.push(p1ToP2);

		const p2ToEcopr =
			p2Date && ecoprDate ? diffDaysFloor(toLocalDayString(p2Date), toLocalDayString(ecoprDate)) : null;
		if (
			p2ToEcopr !== null &&
			p2ToEcopr >= 1 &&
			p2ToEcopr <= MAX_P2_TO_ECOPR_DAYS
		)
			p2ToEcoprDays.push(p2ToEcopr);
	}
	...
	return {
		aorToPal: aorPalDays.length
			? {
					avgDays: Math.round(
						aorPalDays.reduce((a, b) => a + b, 0) / aorPalDays.length,
					),
					sampleCount: aorPalDays.length,
				}
			: null,
		palToFd: palFdDays.length
			? {
					avgDays: Math.round(
						palFdDays.reduce((a, b) => a + b, 0) / palFdDays.length,
					),
					sampleCount: palFdDays.length,
				}
			: null,
		fdToP1: fdToP1Days.length
			? {
					avgDays: Math.round(
						fdToP1Days.reduce((a, b) => a + b, 0) / fdToP1Days.length,
					),
					sampleCount: fdToP1Days.length,
				}
			: null,
		p1ToP2: p1ToP2Days.length
			? {
					avgDays: Math.round(
						p1ToP2Days.reduce((a, b) => a + b, 0) / p1ToP2Days.length,
					),
					sampleCount: p1ToP2Days.length,
				}
			: null,
		p2ToEcopr: p2ToEcoprDays.length
			? {
					avgDays: Math.round(
						p2ToEcoprDays.reduce((a, b) => a + b, 0) / p2ToEcoprDays.length,
					),
					sampleCount: p2ToEcoprDays.length,
				}
			: null,
	};
};
...
const aggregateDashboard = (records) => {
	const aorFrontier = buildStageFrontierByDay(
		records,
		extractAorDate,
		(record, submission, stageDate) => {
			if (!shouldApplyAorLagFilter(record)) return true;
			const lag = diffDaysFloor(
				toLocalDayString(submission),
				toLocalDayString(stageDate),
			);
			return lag !== null && lag >= MIN_AOR_LAG_DAYS;
		},
	);

	const p1Frontier = buildStageFrontierByDay(
		records,
		extractP1Date,
		(_record, submission, stageDate) =>
			isStageLagWithin(submission, stageDate, MAX_P1_LAG_DAYS),
	);

	const fdFrontier = buildStageFrontierByDay(
		records,
		extractFdDate,
		(_record, submission, stageDate) =>
	...
	const p2Frontier = buildStageFrontierByDay(
		records,
		extractP2Date,
		(_record, submission, stageDate) =>
	...
	const ecoprFrontier = buildStageFrontierByDay(
		records,
		extractEcoprDate,
		(_record, submission, stageDate) =>
	...
	const prCardFrontier = buildStageFrontierByDay(
		records,
		extractPrCardDate,
		(record, submission, stageDate) =>
...
const main = async () => {
	...
	for (const source of SOURCES) {
		...
		process.stdout.write(
			`Fetched ${source.key}: ${result.records.length} records\n`,
		);
	}
	...
	await writeFile(
		join(process.cwd(), 'data/clean', 'records.json'),
