...
const abbreviations = [
	...
	[
		'CHC',
		'Canadian High Commission - refers to the Canadian embassies, Visa offices, and Consulates.',
	],
	['CIC', 'Citizenship and Immigration, Canada'],
	[
		'CIO',
		'Centralized Intake Office in Sydney Nova Scotia - where you sent your application. Sometimes it is mentioned as CIU (Centralized Intake Unit).',
	],
...
const translations = {
	zh: {
		...
		langBtn: 'EN',
		disclaimer:
			'本看板数据来源于公开共享的 Google Sheets，仅供参考，不构成任何移民建议。实际情况可能因个案而异，请以官方渠道信息为准。',
		remarksTitle: '平均处理时间',
		remarksTip: '基于数据源过滤异常值后取平均。',
		avgDaysUnit: '天（平均）',
		sampleCount: (n) => `${n} 个样本`,
	},
	en: {
		...
		abbrTip:
			'Sorted alphabetically, with multiple meanings for duplicate abbreviations.',
		abbrCol1: 'Abbr.',
		abbrCol2: 'Full Name',
		langBtn: '中文',
		disclaimer:
			'Data on this dashboard is sourced from publicly shared Google Sheets and is for reference only. It does not constitute immigration advice. Actual outcomes may vary by case — always refer to official sources.',
		remarksTitle: 'Average Stage Duration',
		remarksTip: 'Computed from source data, with outliers filtered out.',
...
const asLocalTime = (iso) => {
	...
	return date.toLocaleString(currentLang === 'zh' ? 'zh-CN' : 'en-CA', {
		hour12: false,
	});
};
...
const renderRemarksStats = (summary) => {
	...
	const entries = [
		...
		{ label: 'FD → P1', data: stats.fdToP1 },
		{ label: 'P1 → P2', data: stats.p1ToP2 },
		{ label: 'P2 → eCOPR', data: stats.p2ToEcopr },
	];
...
const renderDashboard = (summary) => {
	...
	stageLabels.forEach(([key, title]) => {
		...
		const card = document.createElement('article');
		card.className = 'card';
		const stageLabel = currentLang === 'zh' ? '更新日期' : 'Updated';
		card.innerHTML = `
      <h3>${title}</h3>
	...
	const sortedReasons = [...(summary.returns.reasons || [])].sort(
		(a, b) => b.count - a.count,
	);
	...
	if (!sortedReasons.length) {
		...
		empty.textContent =
			currentLang === 'zh' ? '暂无可用原因' : 'No data available';
